...
void SystemInit(void) {
    ...
    // Blue Pill boards typically have 8MHz crystal
    RCC->CR |= RCC_CR_HSEON;
    uint32_t timeout = 100000;
    while (!(RCC->CR & RCC_CR_HSERDY) && timeout--) {}  // Wait for HSE ready

    if (RCC->CR & RCC_CR_HSERDY) {
        // HSE available: 8MHz * 9 = 72MHz
        RCC->CFGR = (RCC->CFGR & ~RCC_CFGR_PLLMULL) | RCC_CFGR_PLLMULL9;  // PLL multiplier = 9
        RCC->CFGR |= RCC_CFGR_PLLSRC;  // PLL source = HSE
    } else {
        // HSE failed, use HSI/2: 4MHz * 16 = 64MHz (max with HSI)
        RCC->CFGR = (RCC->CFGR & ~RCC_CFGR_PLLMULL) | RCC_CFGR_PLLMULL16;  // PLL multiplier = 16
    ...
    RCC->CFGR = (RCC->CFGR & ~RCC_CFGR_PPRE1) | RCC_CFGR_PPRE1_DIV2;  // APB1 = 36MHz
